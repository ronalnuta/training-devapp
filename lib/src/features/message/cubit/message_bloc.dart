...
class MessageBloc extends Cubit<MessageState> {
  MessageBloc() : super(MessageState.ds()) {
    getListMessage(state.pageNumber);
  ...
  void addListenerController() {
    scrollController.addListener(() async {
      if (scrollController.position.pixels ==
        ...
        await getListMessage(state.pageNumber + 1);
        _emitIfOpen(state.copyWith(isLoadingMore: false));
      }
    });
  }

  Future<void> getListMessage(int pageNumber) async {
    ...
    final value = await _domain.messageRepository.getListMessage(
      token: token,
  ...
  void _emitIfOpen(MessageState newState) {
    if (!isClosed) {
      emit(newState);
    }
  }
